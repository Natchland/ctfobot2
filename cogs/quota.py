...
GLOBAL_LB_SQL  = "SELECT user_id,SUM(amount) total FROM quota_submissions WHERE reviewed=TRUE AND week_ts=CURRENT_DATE GROUP BY user_id ORDER BY total DESC LIMIT $1"
PURGE_OLD_SQL  = "DELETE FROM quota_submissions WHERE week_ts < CURRENT_DATE - INTERVAL '8 days'"

# ────────────────── ROLE HELPERS ──────────────────
def _member_has_role(member: discord.Member, *role_ids: int) -> bool:
    want = set(role_ids)
...
def role_check(*role_ids: int) -> Callable:
    ...
    async def predicate(inter: discord.Interaction) -> bool:
        if isinstance(inter.user, discord.Member) and _member_has_role(inter.user, *want):
            return True
...
class QuotaCog(commands.Cog):
    ...
    async def _refresh_task_embed(self, conn, task):
        ...
        prog: Dict[str,int] = {}
        for n in needs:
            val = await conn.fetchval("SELECT COALESCE(SUM(amount),0) FROM quota_submissions WHERE reviewed=TRUE AND resource=$1 AND id>$2", n["resource"], start)
            prog[n["resource"]] = val or 0
        completed = all(prog[n["resource"]] >= n["required"] for n in needs)

        lb = await conn.fetch(
            "SELECT user_id,SUM(amount) total FROM quota_submissions WHERE reviewed=TRUE AND id>$1 GROUP BY user_id ORDER BY total DESC LIMIT $2",
            start, TASK_TOP_LIMIT)

        embed = discord.Embed(title=("✅ COMPLETED" if completed else f"Task: {task['name']}"),
        ...
        for n in needs:
            ...
            embed.add_field(name=n["resource"].title(),
                            value=f"`{have:,}` / `{n['required']:,}`\n{self._bar(have, n['required'])}",
                            inline=False)

        if lb:
            body = "\n".join(f"**{i+1}.** <@{r['user_id']}> `{r['total']:,}`" for i, r in enumerate(lb))
    ...
    async def refresh_weekly_leaderboard(self):
        async with self.db.pool.acquire() as conn:
            ...
            rows = await conn.fetch(GLOBAL_LB_SQL, GLOBAL_LB_SIZE)
            if not rows:
                embed = discord.Embed(title="Weekly Leaderboard", description="No reviewed submissions yet.",
                                      colour=discord.Color.orange())
            else:
                top_total = rows[0]["total"]
                embed = discord.Embed(title="Weekly Leaderboard", colour=discord.Color.purple())
                for i, r in enumerate(rows, start=1):
                    embed.add_field(name=f"#{i}  <@{r['user_id']}>",
                                    value=f"`{r['total']:,}` pts\n{self._bar(r['total'], top_total)}",
                                    inline=False)
            if msg:
                await msg.edit(embed=embed)
            else:
    ...
    @quota.command(name="leaderboard", description="Personal weekly leaderboard")
    @app_commands.choices(resource=[app_commands.Choice(name=r, value=r) for r in RESOURCES])
    async def leaderboard(self, inter: discord.Interaction, resource: app_commands.Choice[str]):
        await self._table_ready.wait()
        rows = await self.db.pool.fetch(
            "SELECT user_id,SUM(amount) total FROM quota_submissions WHERE reviewed=TRUE AND resource=$1 AND week_ts=CURRENT_DATE GROUP BY user_id ORDER BY total DESC LIMIT 15",
            resource.value)
        if not rows:
            return await inter.response.send_message("No data yet.", ephemeral=True)
        out = "\n".join(f"**{i+1}.** <@{r['user_id']}> `{r['total']:,}`" for i, r in enumerate(rows, start=1))
    ...
    async def task_create(self, inter: discord.Interaction, name: str, details: str):
        ...
        async with self.db.pool.acquire() as conn:
            ...
            task_id  = await conn.fetchval("INSERT INTO quota_tasks (name,start_sub,message_id) VALUES ($1,$2,$3) RETURNING id",
                                           name, start_id, msg.id)
            for res, need in pairs.items():
                await conn.execute("INSERT INTO quota_task_needs (task_id,resource,required) VALUES ($1,$2,$3)",
                                   task_id, res, need)
    ...
    async def reset(self, inter: discord.Interaction):
        embed = discord.Embed(title="⚠️ Wipe all quota data?",
                              description="Deletes every submission, task, leaderboard message.",
